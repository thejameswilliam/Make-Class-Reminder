...
class makeReminder
{
    ...
    public function send_instructor_reminder_email() {
        $events = $this->get_weekly_events();
        foreach ($events as $event) {
            $instructors = $this->get_event_instructors($event->ID);
            if($instructors){
                foreach ($instructors as $instructor) {
                    $this->send_instructor_email($instructor, $event->ID);
                }
            }
        }
    }
    ...
    private function send_instructor_email($instructor, $event) {
        $to = $instructor->data->user_email;

        $subject = 'You\'re Teaching: ' . get_the_title($event);
    ...
    private function send_attendee_email($attendee, $occurance) {

        $to = $attendee;

        $occurance_date = new DateTimeImmutable(get_post_meta($occurance, 'event_time_stamp', true));
        $occurance_parent = wp_get_post_parent_id($occurance);
        $event = get_post($occurance_parent);

        $subject = 'You\'re Attending: ' . get_the_title($event) . ' at ' . $occurance_date->format('D, M j @ g:i a');
        $message = $this->open_email_container_html();
        $message .= $this->email_container_styles();
        $message .= '<h1>Hi ' . $attendee->data->display_name . ',<br><br></h1>';
        $message .= '<p>This is a reminder that you\'re attending the following event:</br>';
        // $message .= 'Event: <a href="' . get_permalink( $event ) . '" target="_blank">' . get_the_title($event) . '</a></br>';
        // $message .= (function_exists('tribe_get_start_date') ? 'Date: ' .tribe_get_start_date( $event, true) . '</br>' : 'Date: ' . get_post_meta(  $event, '_EventStartDate', true )) . '</br>';
        // $message .= 'Location: ' . tribe_get_venue( $event ) . '</br></br>';
        // $message .= 'Please let us know if you need anything ahead of time.</p>';
